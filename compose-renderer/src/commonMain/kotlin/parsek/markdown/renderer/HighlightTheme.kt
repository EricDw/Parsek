    ...
    companion object {
        val Default: HighlightTheme = HighlightTheme(
            mapOf(
                ...
                // GFM — tables
                TokenType.TableDelimiter to SpanStyle(color = Color(0xFF808080)),
                TokenType.TableHeaderCell to SpanStyle(color = Color(0xFFD4D4D4)),
                TokenType.TableCell to SpanStyle(color = Color(0xFFD4D4D4)),

                // GFM — strikethrough
                TokenType.StrikethroughMarker to SpanStyle(color = Color(0xFF569CD6)),

                // GFM — task list
                TokenType.TaskMarker to SpanStyle(color = Color(0xFF569CD6)),

                // GFM — extended autolinks
                TokenType.ExtendedAutolinkUrl to SpanStyle(color = Color(0xFF4EC9B0)),
            ),
        )
    }
}
