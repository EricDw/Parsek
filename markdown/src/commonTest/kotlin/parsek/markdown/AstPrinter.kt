    ...
    when (block) {
        ...
        is Block.Table -> {
            ...
            for (cell in block.header.cells) {
                sb.append("$indent    Cell\n")
                for (inline in cell.inlines) appendInline(sb, inline, "$indent      ")
            }
            for ((i, row) in block.body.withIndex()) {
                sb.append("$indent  Row[$i]\n")
        ...
        is Block.TableRow -> {
            sb.append("${indent}TableRow\n")
        }

        is Block.TableCell -> {
            sb.append("${indent}TableCell\n")
        }
    }
}
...
private fun appendInline(sb: StringBuilder, inline: Inline, indent: String) {
    when (inline) {
        ...
        is Inline.Strikethrough -> {
            sb.append("${indent}Strikethrough\n")
            for (child in inline.children) appendInline(sb, child, "$indent  ")
        }

        is Inline.ExtendedAutolink ->
            sb.append("${indent}ExtendedAutolink \"${inline.url}\"\n")
    }
}
